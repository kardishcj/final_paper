...
```
In the case of white men with only a high-school education the model predicts essentially the same starting point of around 37 percent for Millennials and Baby Boomers at the lowest income range of 0-$25,000. But the Millennial in this case quickly surpasses the Baby Boomer in probability of voting in 2012, even if not by a substantial percentage, ultimately topping out at about 60 percent at the highest level of income. This likely reflects the heightened importance of income for the Millennials in terms of predicting 2012 voter turnout.

Then try out other stuff generations on the white guy question before lastly moving to the voting thing.

Then go into the assumption of whether someone voted or not, and run it at every generation. Then close things out. Continue comparison Baby Boomer and Millennial first, then show the other two generations.  
...
ci.plot(simZ_genY_voted, ylim = 0:1, ylab = "Probability of voting", 
        main = "White Millennial Man w/ Past Voting")
...
ci.plot(simZ_genX_voted, ylim = 0:1, ylab = "Probability of voting", 
        main = "White Gen X Man w/ Past Voting")
...
ci.plot(simZ_boomer_voted, ylim = 0:1, ylab = "Probability of voting", 
        main = "White Baby Boomer Man w/ Past Voting")
```
