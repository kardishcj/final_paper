...
#We renamed the levels pertaining to if someone voted and then we coded them to be binary, 
#keeping only Yes/no, changes the variable to a numeric dummy, with 1 as "Yes"
#table(anes$postvote_rvote)
levels(anes$postvote_rvote) <- c("Refused", "Don't know", "Incomplete", 
                                 "Nonresponsive", "Missing", "Inapplicable", 
                                 "No", "No", "No", "Yes")
...
levels(anes$dem_edugroup_x) <- c("Refused", "Don't know", "Data missing", 
                                 "Below high school", "High school", "Some post-high", 
                                 "Bachelor", "Graduate") 
...
levels(anes$dem_raceeth_x) <- c("Data missing", "White", "Black", "Asian" , 
                                "Native American or Alaska Native", "Hispanic","Other") 
...
#We renamed the labels pertaining to voting in the 2008 election, dropped NA ones, 
#and recoded #the remaining yes and no to be 1 or 0. 
...
anes_small <- data.frame(anes$caseid, anes$dem_age_r_x, anes$dem_edugroup_x, anes$black, 
                         anes$hispanic, anes$income, anes$female, anes$interest_voted2008, 
                         anes$postvote_rvote, anes$weight_full, 
                         anes$psu_full, anes$strata_full)

# We named the variables so they are very easy to understand 
colnames(anes_small) <- c("caseID", "age", "education", "black","hispanic", "income", 
                          "female", "vote_2008", "vote_2012", "weights", 
                          "psu", "strata" )
...
ANESdesign_genY <- svydesign(~psu, strata = ~strata , data = anes_genY, 
                             weights = ~weights, variables = NULL, nest = TRUE)

ANESdesign_genX <- svydesign(~psu, strata = ~strata, data = anes_genX,
                             weights = ~weights, variables = NULL, nest = TRUE)

ANESdesign_boomer <- svydesign(~psu,  strata = ~strata, data = anes_boomer, 
                               weights = ~weights, variables = NULL, nest = TRUE)

ANESdesign_silent <- svydesign(~psu, strata = ~strata, data = anes_silent, 
                               weights = ~weights, variables = NULL, nest = TRUE)

ANESdesign_overall <- svydesign(~psu, strata = ~strata, data = anes_small, 
                                weights = ~weights, variables = NULL, nest = TRUE)
...
#Millennials
M_genY <- svyglm(vote_2012 ~ education + female + vote_2008 + black + 
                   hispanic + income, 
                 design = ANESdesign_genY, family = "quasibinomial") 
summary(M_genY)
genY_coef <- coef(M_genY)
exp(genY_coef)

#Gen X
M_genX <- svyglm(vote_2012 ~ education + female + vote_2008 + black + 
                   hispanic + income, design = ANESdesign_genX, 
...
#Baby Boomers
M_boomer <- svyglm(vote_2012 ~ education + female + vote_2008 + black + 
                     hispanic + income, design = ANESdesign_boomer, 
                   family = "quasibinomial") 
summary(M_boomer)
...
#Silent
M_silent <- svyglm(vote_2012 ~ education + female + vote_2008 + black + 
                     hispanic + income, design = ANESdesign_silent, 
                   family = "quasibinomial") 
summary(M_silent)
